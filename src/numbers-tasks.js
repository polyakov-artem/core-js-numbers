...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorOneLength = Math.hypot(x1, y1);
  const vectorTwoLength = Math.hypot(x2, y2);

  const result = Math.acos(scalarProduct / (vectorOneLength * vectorTwoLength));
  return result;
}
...
function getSumOfNumbers(...args) {
  return args.reduce((total, cur) => Number((total + cur).toFixed(1)), 0);
}
